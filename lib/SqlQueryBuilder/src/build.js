class Build {
    ...
    buildInsert(table, params, conn) {
        ...
        x = 0;
        // ADD ? FOR EACH PARAM
        if(conn.toLowerCase() == "postgres" || conn.toLowerCase() == "postgre"
        || conn.toLowerCase() == "postgresql") {
            while (x < params.length) {
                if(x != params.length - 1) 
                    this.query += `$`+(x+1)+`, `;
                else
        ...
        } else if (conn.toLowerCase() == "mysql") {
            while (x < params.length) {
                if(x != params.length - 1) 
                    this.query += `?, `;
                else
    ...
    buildSelect(table, params, clauses, limit, offset, orderBy, conn) {
        var x              = 0;
    ...
    buildUpdate(table, params, clauses, limit, offset, orderBy, conn) {
        var x              = 0;
        ...
        } else if (conn.toLowerCase() == "postgres" || conn.toLowerCase() == "postgre"
        || conn.toLowerCase() == "postgresql") {
            while (x < params.length) {
                if(x != params.length - 1) 
                    this.query += `${params[x]} = $`+(x+1)+`, `;
                else
                    this.query += `${params[x]} = $`+(x+1);
    ...
    buildDelete(table, clauses, limit, offset, orderBy, conn) {
    ...
    buildCount(table, params, clauses, limit, offset, orderBy, conn) {
