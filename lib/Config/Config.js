...
class Config {
    ...
    load( dataString ) {
        ...
        const databases = eval("this.configurationFile.database");

        if(dataString == null && Object.keys(databases).length > 1)
            throw new Error(`App.load() requires a specified database when configure.json has more than one configured!`);

        else if(dataString == null && Object.keys(databases).length == 1)
            dataString = Object.keys(databases)[0];
