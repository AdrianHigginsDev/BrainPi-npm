...
class Shock extends ShockPattern {
    ...
    static connector() {
        ...
        if(dataNode == "" || dataNode == null)
            if(Object.keys(configurationData.database).length > 1)
                throw new Error(`${this.name} requires a specified database in the props function when configure.json has more than one configured!`);
            else
    ...
    connector() {
        ...
        if(dataNode == "" || dataNode == null)
            if(Object.keys(configurationData.database).length > 1)
                throw new Error(`${classObj.name} requires a specified database in the props function when configure.json has more than one configured!`);
            else
                dataNode = Object.keys(configurationData.database)[0];
